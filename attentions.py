...
class MultiHeadLocationAwareAttention(nn.Module):
    ...
    Reference:
        ...
        - **Attention-Based Models for Speech Recognition**: https://arxiv.org/abs/1506.07503
    """
    def __init__(self, seq_len, hidden_dim: int, num_heads: int = 8, conv_out_channel: int = 10) -> None:
        super(MultiHeadLocationAwareAttention, self).__init__()
        self.hidden_dim = hidden_dim
        self.num_heads = num_heads
        self.dim = int(hidden_dim / num_heads)
        self.conv1d = nn.Conv1d(num_heads, conv_out_channel, kernel_size=3, padding="same")
...
#         batch_size, seq_len = value.size(0), value.size(1)
        batch_size, seq_len = value.size(0), seq_len
